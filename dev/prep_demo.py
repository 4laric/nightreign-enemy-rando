...
prep_demo.py — Two related jobs in one tool:
  ...
  RANK: score every spoiler in a seed dir on demo-fitness and print the
    ...
    seed to record against. Uses all 5 criteria; requires callsites.json.
  ...
  RANK-LITE: tile-aware seed ranking that does NOT require callsites.json
    or any EMEVD decompile. Uses only NB arena distinctiveness and
    tile-weighted memorable field placements (criteria 1 & 4). Pick this
    when you want "which seed has visually interesting rolls" rather than
    "which seed is best for the healthbar-rename demo workflow."

      python prep_demo.py RANK-LITE \
          --seeds-root seeds/ \
          --top 10 \
          --tile-weighting vanilla
...
Demo-fitness scoring criteria, in order of weight (see _score_spoiler):
  ...
  3. Heterogeneous squad tiebreaker: bonus for at least one shared-bar
     ...
     the "demo moment" the squad-annotation feature exists for. (RANK only.)
  4. Memorable field placements: bonus if specific recognizable field
     ...
     --tile-weighting {off|uniform|vanilla}; default is 'vanilla'. (Both
     modes.)
  5. Callsite variety: how many distinct chrs land at active callsites.
     (RANK only.)
...
def _score_spoiler(spoiler_path, callsites, tile_prob=TILE_PROB_OFF):
    """Return (score, breakdown_dict, seed).

    `callsites` may be None (or an empty list) — in that case the three
    EMEVD-dependent criteria (nameid_reuse_safety, heterogeneous_squads,
    callsite_variety) are skipped and the score uses only NB arena
    distinctiveness + tile-weighted memorable-field placements. This is
    the RANK-LITE path and is purely spoiler-driven; it doesn't require
    callsites.json. The full RANK path uses all five criteria.
    ...
    # Lite path: caller passes callsites=None to skip EMEVD-dependent criteria.
    # Empty list (e.g. degenerate RANK call) still goes through the full path
    # so the breakdown dict gets all 5 keys.
    use_callsites = callsites is not None
    ...
    # --- Criteria 2, 3, 5 are EMEVD-dependent. Skip when no callsites loaded. ---
    n_colliding = 0
    ...
    # --- Criterion 5: Overall variety at callsites (EMEVD-dependent) ---
    ...
    # Weighted total. Caps prevent any one criterion from dominating. The
    # memorable-field term uses tile-weighted sum (field_weighted, not
    # field_distinct) so rare-tile placements get discounted by how often
    # the viewer actually sees them. When use_callsites is False (RANK-LITE
    # path), the three EMEVD-dependent terms zero out and the score sums
    # only NB arena distinctiveness + tile-weighted memorable field.
    score = (
...
def _resolve_tile_prob_table(args):
    """Resolve the empirical msb_tile_probability.json source. Returns (count, mode_dict).

    Search order (first match wins):
      1. --msb-tile-prob FLAG (explicit)
      2. msb_tile_probability.json next to the script
      3. data/msb_tile_probability.json (sibling dir of the script — handles
         the layout where prep_demo.py lives at the project root)
      4. ../data/msb_tile_probability.json (parent's data dir — handles the
         layout where prep_demo.py lives in healthbar_tools/)
      5. cwd's data/msb_tile_probability.json (final fallback for "I cd'd
         into the project root and ran it from somewhere weird")
    """
    n_empirical = 0
    if args.msb_tile_prob:
        ...
        cwd = os.getcwd()
        candidates = [
            os.path.join(here, 'msb_tile_probability.json'),
            os.path.join(here, 'data', 'msb_tile_probability.json'),
            os.path.join(here, '..', 'data', 'msb_tile_probability.json'),
            os.path.join(cwd, 'data', 'msb_tile_probability.json'),
        ]
    ...
    return n_empirical, tile_prob


def _rank_seeds(seeds_root, callsites, tile_prob):
    """Score every spoiler in seeds_root. Returns (candidates, n_scanned, n_failed).

    `callsites` may be None for the lite path; _score_spoiler will skip
    EMEVD-dependent criteria when it's None or empty.
    """
    if not os.path.isdir(seeds_root):
        sys.stderr.write(f"ERROR: --seeds-root {seeds_root!r} doesn't exist or isn't a directory.\n")
        sys.stderr.write(f"       Did you run simulate_seeds.py yet?\n")
        return [], 0, 0
    candidates = []
    n_scanned = 0
    n_failed = 0
    n_dirs = 0
    for entry in sorted(os.listdir(seeds_root)):
        full = os.path.join(seeds_root, entry)
    ...
    if n_scanned == 0:
        if n_dirs == 0:
            sys.stderr.write(f"WARNING: {seeds_root!r} is empty. Run simulate_seeds.py first.\n")
        else:
            sys.stderr.write(f"WARNING: found {n_dirs} subdirs under {seeds_root!r} but none "
                             f"contain _spoilers.json. Did simulate_seeds.py finish, or are the "
                             f"spoilers under a different filename?\n")
    candidates.sort(key=lambda c: -c['score'])
    return candidates, n_scanned, n_failed
...
def cmd_rank(args):
    ...
    n_empirical, tile_prob = _resolve_tile_prob_table(args)
    candidates, n_scanned, n_failed = _rank_seeds(args.seeds_root, callsites, tile_prob)
...
def cmd_rank_lite(args):
    """Tile-aware seed ranking that doesn't require callsites.json.

    Uses only the two spoiler-driven criteria: NB arena distinctiveness
    (4.0 × min(count, 12)) and tile-weighted memorable field placements
    (1.0 × min(weighted, 6.0)). No EMEVD dependency, no upfront SCAN step.
    Use this when you want "which seed has visually interesting tile rolls"
    rather than "which seed is best for the healthbar-rename demo."
    """
    n_empirical, tile_prob = _resolve_tile_prob_table(args)
    candidates, n_scanned, n_failed = _rank_seeds(args.seeds_root, None, tile_prob)

    sys.stderr.write(f"Scanned {n_scanned} spoilers ({n_failed} failed)\n")
    sys.stderr.write(f"Mode: RANK-LITE (no callsites; NB arena + memorable field only)\n")
    sys.stderr.write(f"Tile weighting: {args.tile_weighting}\n")
    if n_empirical:
        sys.stderr.write(f"Empirical per-MSB tile presence loaded: {n_empirical} MSBs\n")
    else:
...
def main():
    ...
    r = sub.add_parser('RANK',
                       help='Score seeds for the healthbar-renaming demo. Needs '
                            'callsites.json. Uses all 5 criteria.')
    ...
    r.add_argument('--msb-tile-prob', default=None,
                   help="Path to msb_tile_probability.json with empirical per-MSB per-tile "
                        ...
                        "msb_tile_probability.json next to prep_demo.py and then in "
                        "../data/. If neither exists, falls back to the prefix heuristic in "
                        "_msb_tiles().")
    r.set_defaults(func=cmd_rank)

    rl = sub.add_parser('RANK-LITE',
                        help='Tile-aware seed ranking without callsites. Uses only NB arena '
                             'distinctiveness + tile-weighted memorable field. No EMEVD scan '
                             'required.')
    rl.add_argument('--seeds-root', required=True)
    rl.add_argument('--top', type=int, default=10)
    rl.add_argument('--tile-weighting', choices=['off', 'uniform', 'vanilla'],
                    default='vanilla',
                    help="Same options as RANK --tile-weighting; default 'vanilla'.")
    rl.add_argument('--msb-tile-prob', default=None,
                    help="Same as RANK --msb-tile-prob; same auto-discovery search order.")
    rl.set_defaults(func=cmd_rank_lite)
