"""Public BHD keys for Elden Ring's dvdbnd archives, per archive.

Standard public BHD keys as shipped in UXM / BinderTool — cryptographic
constants (a number), not authored code. Used read-only to read chr / script
/ sfx files out of a copy of Elden Ring you own, for the heritage import.
Archive names match the on-disk .bhd/.bdt basenames (Data0-3 base game, DLC =
Shadow of the Erdtree, sd/* = sound). Forward slashes are used so the same
names work on Windows and POSIX.
"""

ELDEN_RING_KEYS = {
    'Data0': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    'Data1': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    'Data2': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    'Data3': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    'DLC': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    'sd/sd': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    'sd/sd_dlc02': """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
}

# Archives that actually hold chr/script/sfx (sd/* are sound data — harmless to
# index but never needed for heritage imports).
ELDEN_RING_ARCHIVES = ["Data0", "Data1", "Data2", "Data3", "DLC", "sd/sd", "sd/sd_dlc02"]
