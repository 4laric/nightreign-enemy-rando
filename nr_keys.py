"""Public BHD keys for Nightreign's dvdbnd archives, per archive.

Cryptographic constants (the standard public BHD keys, as shipped in
UXM / BinderTool) — a number, not authored code. Nightreign reuses the
Elden Ring BHD5 *format* but carries its own per-archive keys. Used
read-only, to read map files out of a copy of the game you own.
"""

NIGHTREIGN_KEYS = {
    "data0": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    "data1": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    "data2": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    "data3": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
    "sd__sd": """-----BEGIN RSA PUBLIC KEY-----

-----END RSA PUBLIC KEY-----
""",
}
