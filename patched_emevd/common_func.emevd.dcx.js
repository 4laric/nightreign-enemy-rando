...
$Event(90015442, Restart, function(entityId, eventFlagId) {
    ...
    if (EntityInRadiusOfEntity(1028402601, entityId, 15, 1)) {
        WaitFor(EventFlag(1028400205) || ElapsedSeconds(90));
    }
    if (EntityInRadiusOfEntity(1028402602, entityId, 15, 1)) {
        WaitFor(EventFlag(1028400210) || ElapsedSeconds(90));
    }
    if (EntityInRadiusOfEntity(1056402601, entityId, 15, 1)) {
        WaitFor(EventFlag(1056400200) || ElapsedSeconds(90));
    }
    if (EntityInRadiusOfEntity(1056402602, entityId, 15, 1)) {
        WaitFor(EventFlag(1056400205) || ElapsedSeconds(90));
    }
    if (EntityInRadiusOfEntity(1056402603, entityId, 15, 1)) {
        WaitFor(EventFlag(1056400210) || ElapsedSeconds(90));
    }
    SetNetworkconnectedEventFlagID(eventFlagId, ON);
});
...
L0:
    ...
    WaitFixedTimeSeconds(2);
    PlaySE(chrEntityId, SoundType.SFX, 888880000);
    WaitFor(CharacterDead(chrEntityId) || ElapsedSeconds(5));
    GotoIf(S0, value == 3);
    if (value != 2) {
        if (value != 1) {
...
$Event(99055200, Restart, function(triggerFlag, bossEntity1, bossEntity2) {
    ...
    // Phase 4: force-enable this arena's boss entity (and
    // head2 for duo arenas). Idempotent: EnableCharacter on
    // an already-enabled chr is a no-op; on a nonexistent
    // entity is also a no-op.
    EnableCharacter(bossEntity1);
    EnableCharacterCollision(bossEntity1);
    EnableCharacterAI(bossEntity1);
    RequestCharacterAIReplan(bossEntity1);
    EnableCharacter(bossEntity2);
    EnableCharacterCollision(bossEntity2);
    EnableCharacterAI(bossEntity2);
    RequestCharacterAIReplan(bossEntity2);

    // Phase 5 (audible): post-fire confirmation.

    // Phase 6: hold until encounter ends. Restart mode
    // re-arms on the next encounter (next map / death).
    WaitFor(!EventFlag(triggerFlag));
});

$Event(99055300, Restart, function(anchorEntity, triggerFlag) {
    ...
    WaitFor(EntityInRadiusOfEntity(20000, anchorEntity, 20, 1));
    EndIf(EventFlag(triggerFlag));
    SetNetworkconnectedEventFlagID(triggerFlag, ON);
});

$Event(99055400, Restart, function(anchorEntity, triggerFlag) {
    // nb_arena_hold_trigger (v0.24.111)
    //
    // Player-deliberate arena trigger: stand within
    // 25m of arena center for 3 continuous seconds.
    ...
    WaitFor(EntityInRadiusOfEntity(20000, anchorEntity, 25, 1));
    ...
    WaitFor(ElapsedSeconds(3) || !EntityInRadiusOfEntity(20000, anchorEntity, 25, 1));
    EndIf(!EntityInRadiusOfEntity(20000, anchorEntity, 25, 1));
