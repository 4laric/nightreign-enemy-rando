...
$Event(90065900, Restart, function(entityId, value, assetEntityId, chrEntityId, textEffectParamId, bgmBossConvParamId, logObjectId, entityId2) {
    ...
    if (IsMapVariation(0)) {
        // OOPS_ALL_NB fix: progression flag now follows the SLOT's night,
        // not this arena's hardcoded `value`. 7506 (night-2 active) is set
        // once night 2 begins and is off through all of night 1, so it
        // selects 7502 (-> day 2) vs 7507 (-> day 3) by the night actually
        // being played -- correct for any boss placed in this slot. Death/
        // reward/grace/repulsed above are unchanged. `value` is now vestigial.
        GotoIf(S0, EventFlag(7506));
        SetNetworkconnectedEventFlagID(7502, ON);
        Goto(L0);
S0:
        GotoIf(S1, !EventFlag(7506));
...
//  player proximity to the night boss (within 5m), so a player can start the
...
$Event(90065950, Default, function(bossEntityId, arenaFlag, gateFlag, night) {
    DisableNetworkSync();
    ...
    if (night == 1) {
        ...
        WaitFor(EntityInRadiusOfEntity(20000, bossEntityId, 5, 1)
            || EventFlag(arenaFlag));
    }
    if (night == 2) {
        WaitFor((EntityInRadiusOfEntity(20000, bossEntityId, 5, 1)
