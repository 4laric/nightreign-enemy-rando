...
class TestFirstLaunchWizardStructure:
    ...
    def test_has_two_screens(self, gui_source):
        """SCREEN_NAMES must declare both screens. If a future edit
        adds/drops one, the integration test for main() may also need
        to update — fail loud here. (The Oodle screen was removed once
        Oodle resolution moved to an on-demand check at run time; the
        output screen was removed once output became an auto-scaffolded
        local me3 profile instead of a prompted path.)"""
        # The class-level attribute should be present as a list literal
        # with exactly these 2 names in order
        idx = gui_source.find('class FirstLaunchWizard:')
        snippet = gui_source[idx:idx + 2000]
        assert "SCREEN_NAMES = ['welcome', 'done']" in snippet

    def test_all_screen_builders_exist(self, gui_source):
        for screen in ('welcome', 'done'):
