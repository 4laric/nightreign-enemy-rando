...
class TestInstallMdGuidance:
    def test_install_mentions_tricephalos(self, install_source):
        """INSTALL.md should mention the recommendation in the launch
        step — that's where users will be looking when they're about
        to start an expedition."""
        assert 'Tricephalos' in install_source, (
            'INSTALL.md missing Tricephalos guidance — users reading '
            'docs won\'t know about the EMEVD gap.')

    def test_install_explains_why(self, install_source):
        """Don't just say 'pick Tricephalos' — explain it's because of
        Night Boss EMEVD validation status, so users understand the
        guidance is time-bound and not permanent."""
        # Look near the Tricephalos mention for an explanation
        idx = install_source.find('Tricephalos')
        assert idx != -1
        window = install_source[max(0, idx - 200):idx + 800]
        # Should mention EMEVD or validation as the rationale
        assert ('EMEVD' in window or 'validated' in window
                or 'compatibility' in window), (
            'INSTALL.md\'s Tricephalos recommendation should explain '
            'WHY (Night Boss EMEVD validation in progress). Bare '
            'recommendations without rationale look arbitrary.')

    def test_install_under_launch_step(self, install_source):
        """The recommendation should live in step 5 (Launch through me3)
        — that's the natural moment for the guidance."""
        launch_idx = install_source.find('Launch through me3')
        tricephalos_idx = install_source.find('Tricephalos')
        next_section_idx = install_source.find('## When things go wrong')
        assert launch_idx != -1 and tricephalos_idx != -1
        assert launch_idx < tricephalos_idx, (
            'Tricephalos guidance should come after the "Launch through '
            'me3" section header.')
        if next_section_idx != -1:
            assert tricephalos_idx < next_section_idx, (
                'Tricephalos guidance should live within the Launch '
                'section, not leak into "When things go wrong".')
